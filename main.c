...
int main() {
    ...
    printf("Type your RISC-V instructions directly and press Enter to execute.\n");
    printf("Type 'exit' to quit the application.\n\n");

    // INTERACTIVE REPL LOOP
    while (1) {
        // Clear screen noise, just read directly without printing "Instruction:"
        ...
        char program_storage[1][50];
        const char* program_pointers[1];
        strcpy(program_storage[0], user_input);
        program_pointers[0] = program_storage[0];

        // TWO-PASS ASSEMBLER
        assembler_pass1(program_pointers, 1);

        char check_label[50];
        if (sscanf(program_pointers[0], "%s", check_label) == 1) {
            if (check_label[strlen(check_label) - 1] == ':') {
                // It's just a label line, wait for next instruction
                continue;
            }
        }

        uint32_t machine_code = assemble_line(program_pointers[0], 0);
        if (machine_code == 0) {
            printf("Error: Invalid assembly syntax. Try again.\n\n");
            continue;
        }

        // Reset memory loader bounds for this single instruction execution
        ...
        // Reset CPU pipeline registers for the new execution runtime but KEEP register contents!
        my_cpu.pc = 0;
        my_cpu.IF_ID.valid = 0; my_cpu.IF_ID.instruction = 0; my_cpu.IF_ID.pc = 0;
        my_cpu.ID_EX.valid = 0; my_cpu.ID_EX.pc = 0;
        my_cpu.EX_MEM.valid = 0; my_cpu.EX_MEM.pc = 0; my_cpu.EX_MEM.alu_result = 0;
        my_cpu.MEM_WB.valid = 0; my_cpu.MEM_WB.pc = 0; my_cpu.MEM_WB.final_result = 0;
        ...
        while (my_cpu.pc < load_address || my_cpu.IF_ID.valid || my_cpu.ID_EX.valid || my_cpu.EX_MEM.valid || my_cpu.MEM_WB.valid) {
            ...
            if (my_cpu.ID_EX.valid) {
                ...
                if (op == 0x63 && alu_out == 1) {
                    ...
                    next_IF_ID.valid = 0; next_ID_EX.valid = 0; is_flushed = 1; total_stalled_cycles += 2;
                }
                else if (op == 0x6F) {
                    my_cpu.pc = my_cpu.ID_EX.pc + my_cpu.ID_EX.decoded_inst.imm;
                    next_IF_ID.valid = 0; next_ID_EX.valid = 0; is_flushed = 1; total_stalled_cycles += 2;
                ...
                if (op == 0x33) {
                    ...
                    printf("%d : execute   : [pc=%ld] %s x%d, x%d, x%d [%s %ld, %ld, %ld]\n",
                           clock_cycle, my_cpu.ID_EX.pc, name, my_cpu.ID_EX.decoded_inst.rd, rs1_reg, rs2_reg, name, alu_out, forwarded_rs1, forwarded_rs2);
                } else if (op == 0x63) {
                    printf("%d : execute   : [pc=%ld] %s x%d, x%d, %ld [%s %ld, %ld, %ld] %s\n",
                           clock_cycle, my_cpu.ID_EX.pc, name, rs1_reg, rs2_reg, my_cpu.ID_EX.decoded_inst.imm, name, forwarded_rs1, forwarded_rs2, my_cpu.ID_EX.decoded_inst.imm, (alu_out == 1) ? "-> TAKEN (FLUSH)" : "-> NOT TAKEN");
                } else if (op == 0x6F) {
                    ...
                    printf("%d : execute   : [pc=%ld] %s x%d, x%d, %ld [%s %ld, %ld, %ld]\n",
                           clock_cycle, my_cpu.ID_EX.pc, name, my_cpu.ID_EX.decoded_inst.rd, rs1_reg, my_cpu.ID_EX.decoded_inst.imm, name, alu_out, forwarded_rs1, my_cpu.ID_EX.decoded_inst.imm);
                }
            ...
            if (my_cpu.IF_ID.valid && !is_flushed) {
                ...
                if (op == 0x33) {
                    ...
                    printf("%d : decode    : [pc=%ld] %s x%d, x%d, x%d [%s %ld, %ld, %ld]\n",
                           clock_cycle, my_cpu.IF_ID.pc, name, decoded.rd, decoded.rs1, rs2_reg, name, my_cpu.registers[decoded.rd], my_cpu.registers[decoded.rs1], my_cpu.registers[rs2_reg]);
                } else if (op == 0x63) {
                    printf("%d : decode    : [pc=%ld] %s x%d, x%d, %ld [%s %ld, %ld, %ld]\n",
                           clock_cycle, my_cpu.IF_ID.pc, name, decoded.rs1, decoded.rd, decoded.imm, name, my_cpu.registers[decoded.rs1], my_cpu.registers[decoded.rd], decoded.imm);
                } else if (op == 0x6F) {
                    ...
                    printf("%d : decode    : [pc=%ld] %s x%d, x%d, %ld [%s %ld, %ld, %ld]\n",
                           clock_cycle, my_cpu.IF_ID.pc, name, decoded.rd, decoded.rs1, decoded.imm, name, my_cpu.registers[decoded.rd], my_cpu.registers[decoded.rs1], decoded.imm);
                }
                next_IF_ID.valid = 0;
            }
            ...
            if (my_cpu.pc < load_address && !is_flushed) {
                ...
                if (op == 0x33) {
                    ...
                    printf("%d : fetch     : [pc=%ld] %s x%d, x%d, x%d [%s %ld, %ld, %ld]\n",
                           clock_cycle, my_cpu.pc, name, f_decoded.rd, f_decoded.rs1, rs2_reg, name, my_cpu.registers[f_decoded.rd], my_cpu.registers[f_decoded.rs1], my_cpu.registers[rs2_reg]);
                } else if (op == 0x63) {
                    printf("%d : fetch     : [pc=%ld] %s x%d, x%d, %ld [%s %ld, %ld, %ld]\n",
                           clock_cycle, my_cpu.pc, name, f_decoded.rs1, f_decoded.rd, f_decoded.imm, name, my_cpu.registers[f_decoded.rs1], my_cpu.registers[f_decoded.rd], f_decoded.imm);
                } else if (op == 0x6F) {
                    ...
                    printf("%d : fetch     : [pc=%ld] %s x%d, x%d, %ld [%s %ld, %ld, %ld]\n",
                           clock_cycle, my_cpu.pc, name, f_decoded.rd, f_decoded.rs1, f_decoded.imm, name, my_cpu.registers[f_decoded.rd], my_cpu.registers[f_decoded.rs1], f_decoded.imm);
                }
                my_cpu.pc += 4;
            }
        ...
        printf("Current State -> x5: %ld, x6: %ld, x7: %ld\n", my_cpu.registers[5], my_cpu.registers[6], my_cpu.registers[7]);
        printf("-----------------------------------------\n\n");
    }
